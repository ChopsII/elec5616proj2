...
public class jbStealthNetFileTransfer extends Thread {
	...
	public jbStealthNetFileTransfer(jbStealthNetComms snComms, String fn, boolean b) {
	...
	public void run() {
		...
		JOptionPane.showMessageDialog(ftpFrame, (bSend ? "Upload Complete"
				: "Download Complete"), "StealthNet",
	...
	private synchronized void sendFile() {
		...
		try {
			stealthComms.sendPacket(jbStealthNetPacket.CMD_FTP, Integer
					.toString(fileLen));
			...
			do {
				bufLen = fid.read(buf);
				if (bufLen > 0) {
					stealthComms.sendPacket(jbStealthNetPacket.CMD_FTP, buf,
							bufLen);
					stealthComms.recvPacket();
				}
			...
			fid.close();
			stealthComms.sendPacket(jbStealthNetPacket.CMD_FTP);
		} catch (IOException e) {
	...
	private synchronized void recvFile() {
		...
		try {
			fileLen = (new Integer(new String(stealthComms.recvPacket().data)))
					.intValue();
			stealthComms.sendPacket(jbStealthNetPacket.CMD_NULL);
			progressBar.setMaximum(fileLen);
