...
class jbStealthNetStorage {
	// randomly generated by perfectly random human ie. just a static salt
	// 'cause its not really important
	static byte[] salt = { (byte) 0xBE, (byte) 0xB2, (byte) 0xAC, (byte) 0xA2,
			(byte) 0x0C, (byte) 0xED, (byte) 0xE5, (byte) 0x3C };

	/*
	 * public static void main(String[] args){ store("bens_file",
	 * "mysalt","secretpassword".toCharArray(),
	 * "Oh shit, some hidden bytes.".getBytes()); System.out.println(new
	 * String(retrieve("bens_file", "mysalt","secretpassword".toCharArray())));
	 * }
	 */
	...
	/*
	 * Turns a string into a valid 8 byte salt by truncating it.
	 */
	...
	/*
	 * Performs our Password Based Encryption Input: Mode - Encrypt or Decrypt
	 * Salt - the 8-byte salt to use Password - the password as a char array
	 * Input - the byte data to process Then uses MD5 and DES to secure or
	 * decrypt the input.
	 */
	private static byte[] doPBE(int mode, byte[] salt, char[] password,
			byte[] input) {
	...
	/* Digests a byte array using HMAC-MD5 */
	...
	public static boolean store(String filename, String salt, String password,
			byte[] data) {
		return store(filename, salt, password.toCharArray(), data);
	}

	public static boolean store(String filename, String salt, char[] password,
			byte[] data) {
	...
	/*
	 * Takes our data, encrypts it and then writes it out to a file File format:
	 * <Length of the encrypted data> <Length of the digest> <The encrypted
	 * data> <The digest>
	 */
	public static boolean store(String filename, byte[] salt, char[] password,
			byte[] data) {
		...
		try {
			DataOutputStream out = new DataOutputStream(new FileOutputStream(
					filename));
			out.writeInt(encrypted.length); // store the length of the data we
											// are outputting
			out.writeInt(digest.length);
			out.write(encrypted, 0, encrypted.length); // write out all our
														// encrypted data
	...
	public static byte[] retrieve(String filename, String salt, String password) {
		return retrieve(filename, salt, password.toCharArray());
	}
	...
	/*
	 * Retrieves data back from a file and decrypts it. Also validates that the
	 * MACs match.
	 */
	public static byte[] retrieve(String filename, byte[] salt, char[] password) {
		...
		if (!same(digest, doDigest(encrypted))) {
			System.out
					.println("Possible Hack Attempt: Stored digest differs from actual");
			return null;
		}
