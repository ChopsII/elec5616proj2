...
public class StealthNetFileTransfer extends Thread {
    ...
    public StealthNetFileTransfer(JBStealthNetComms snComms, String fn, boolean b) {
    ...
    private synchronized void sendFile() {
        ...
        try {
            stealthComms.sendPacket(JBStealthNetPacket.CMD_FTP, Integer.toString(fileLen));
            stealthComms.recvPacket();
            fid = new FileInputStream(filename);
            do {
                bufLen = fid.read(buf);
                if (bufLen > 0) {
                    stealthComms.sendPacket(JBStealthNetPacket.CMD_FTP, buf, bufLen);
                    stealthComms.recvPacket();
                }
