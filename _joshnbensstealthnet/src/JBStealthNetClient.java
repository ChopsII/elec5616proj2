...
public class JBStealthNetClient{
    ...
    public Component createGUI() {
        ...
        final JButton creditBtn = new JButton(new ImageIcon("coins.gif"));
        ...
        creditBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) { withdrawCredits(); }
        });
    ...
    private synchronized void login() {
        ...
        try {
            ...
            stealthComms = new JBStealthNetComms();
            stealthComms.initiateSession(new Socket(JBStealthNetComms.SERVERNAME, JBStealthNetComms.SERVERPORT));
            stealthComms.sendPacket(JBStealthNetPacket.CMD_LOGIN, userID);
            stealthTimer.start();
        } catch (UnknownHostException e) {
            msgTextBox.append("[*ERR*] Unknown host: " + JBStealthNetComms.SERVERNAME + "\n");
        } catch (IOException e) {
            msgTextBox.append("[*ERR*] Could not connect to host: " + JBStealthNetComms.SERVERNAME + "\n");
        }
    ...
    private void withdrawCredits(){
        ...
        credits = Integer.parseInt(JOptionPane.showInputDialog("How many credits to withdraw:", credits));

        stealthComms.sendPacket(JBStealthNetPacket.CMD_WITHDRAW, "");

        msgTextBox.append("Success " + credits + "\n");
    }

    private byte[] hashChain(int timesToHash) {
    ...
    private void purchaseSecret(int row) {
		...
		if ((fileSave.getFile() != null) && (fileSave.getFile().length() > 0)) {
			...
			try {
				...
				JBStealthNetComms snComms = new JBStealthNetComms();
            ...
            while (stealthComms.recvReady()) {
                ...
                switch (pckt.command) {
                    ...
                    case JBStealthNetPacket.CMD_WITHDRAW :
                        msgTextBox.append("[INFO] Recieved a withdraw message from server. \n");
                        break;
