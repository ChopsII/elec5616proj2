...
public class StealthNetComms {
	...
	private void doKeyExchangeAlice() throws InvalidKeyException,
		...
		sharedSecretKey = generateKey(aliceKeyAgree.generateSecret());
		...
		myRandom.setSeed(concatByteArray(sharedSecretKey.getEncoded(), "alice"
				.getBytes()));
		theirRandom.setSeed(concatByteArray(sharedSecretKey.getEncoded(), "bob"
				.getBytes()));
	...
	private void doKeyExchangeBob() throws IOException,
		...
		sharedSecretKey = generateKey(sharedSecret);

		// Initialise Alice and bob's PRNGs with the shared secret key and short
		// string to ensure different seed
		myRandom.setSeed(concatByteArray(sharedSecretKey.getEncoded(), "bob"
				.getBytes()));
		theirRandom.setSeed(concatByteArray(sharedSecretKey.getEncoded(),
				"alice".getBytes()));
	...
	/**
	 * This function generates a Blowfish SecretKey based on MD5 hasing the DH
	 * shared secret
	 * 
	 * @param sharedSecret
	 *            The secret agreed upon during Diffie-Helman
	 * @returnThe secret key to use for encryption etc
	 * @throws NoSuchAlgorithmException
	 */
	private SecretKey generateKey(byte[] sharedSecret)
			throws NoSuchAlgorithmException {
		// hash shared secret, generate Blowfish key from that.
		MessageDigest md = MessageDigest.getInstance("MD5");
		SecretKey key = new SecretKeySpec(md.digest(sharedSecret), "Blowfish");
	...
	public boolean sendPacket(StealthNetPacket pckt) {
		...
		if (pckt.command == StealthNetPacket.CMD_KEYEXCHANGE)
			...
			pckt.data = concatByteArray(new byte[]{pckt.command}, pckt.data);
	...
	private byte[] addHMACandNonce(byte[] data) {
		...
		return concatByteArray(hmac, concatByteArray(nonce, data));
	}
